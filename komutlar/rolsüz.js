...
.setColor("RANDOM")
.setDescription("Sunucuda rolü olmayan \`"+ knaveqwe.size +"\` kişiye sunucu üyesi rolü verildi!")
message.channel.send(khold).then(m => message.react((client.config.onayemoji)))
} else if(!args[0]) {
const khold1 = new Discord.MessageEmbed()
.setAuthor(""+message.author.username +" ", message.author.avatarURL())
.setColor("RANDOM")
.setDescription("Sunucumuzda rolü olmayan \`"+ knaveqwe.size +"\` kişi var. Bu kişilere üye rolü vermek için \`.rolsüz ver\` komutunu uygulayın!")
